...
namespace k_spider_sync.transfer;
...
public static class TransferSpiderData
{
    ...
    {
        using var remote = Pg.Connection(TransferConfig.RemoteConnectionString);
        using var local = Pg.Connection(TransferConfig.LocalConnectionString);
